...
library(ggplot2)
carData$odometer <- as.numeric(carData$odometer)
carData$odometer <- factor(carData$odometer)
carData$condition <- factor(carData$condition)
ggplot(carData, aes(x = price, y = condition))+geom_point() + geom_smooth(method = lm) 
g<- ggplot(carData, aes(x = price, y = odometer))
g+geom_jitter(alpha = .5, width = 0.01, color = "blue")+coord_cartesian(xlim = c(4000,100000), ylim= c(0,240000)) +theme_light()
```
```{r}
g+ geom_point(alpha = 0.1, color = "blue")+coord_cartesian(xlim = c(0,10000), ylim= c(0,500500)) +theme_light()
...
carData$price<-as.factor(carData$price)
...
log <- rpart(price ~ region+ year+manufacturer+condition+model + cylinders + fuel + odometer+ title_status+ transmission+drive+ size+ type+paint_color+ state+lat+ long, data = trainingCar, family = binomial(link = "logit"))
```
